...
void ft_draw_points(t_new *new)
{
	...
	if (new->net_flag == 2)
	{	ft_putendl("here");
		ft_draw_triangle(new, new->data_serv, 0);
	}
}

void	ft_draw_screen(t_new *new)
{
	new->sdl.win_ptr = SDL_CreateWindow("TRY ANGLE CLIENT", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, new->win.width, new->win.height, SDL_WINDOW_RESIZABLE );
...
void	ft_main_loop(t_new *new)
{
	...
	while(1)
	{
		...
		if (new->change_fl)
		{
			ft_clear_tri(new, new->data_serv);
			SDL_LockMutex(mutex);
			ft_memcpy(&new->data_serv[0], &new->tmp, sizeof(t_data));
			new->data_serv[0].col =0xAEF100;
			ft_fill_strcut(new->data_serv);
